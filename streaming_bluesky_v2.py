...
def guardar_post_en_json(post_json, carpeta="posts_bluesky"):
    ...
    # # Generar nombre de archivo único usando timestamp y UUID
    # timestamp = post_json.get("timestamp", datetime.utcnow().isoformat())
    # safe_timestamp = timestamp.replace(":", "-")  # evitar ":" en nombres de archivo
    # nombre_archivo = f"{safe_timestamp}_{uuid.uuid4().hex[:8]}.json"

    # ruta_archivo = Path(carpeta) / nombre_archivo
...
def procesar_post_y_descargar_imagen(post, base_name):
    ...
    # uri = post.get("uri", "")
    # post_did = uri.split("/")[2] if "at://" in uri else "unknown"
    # source = post.get("source", "unknown")
    # timestamp = post.get("timestamp", datetime.utcnow().isoformat()).replace(":", "-")
    ...
    #     filename = f"{source}_{timestamp}_{idx}.jpg"
    ...
    #     print(f"✅ Imagen guardada: {filename} | {width}x{height}px, {size_kb:.1f} KB")
    media_items = post.get("media_urls", [])
